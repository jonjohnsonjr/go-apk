...
func (a *APK) installRegAndLinks(ctx context.Context, in io.Reader, origin, replaces string, headers map[string]tar.Header, indexes []map[string]tar.Header) ([]tar.Header, error) { //nolint:gocyclo
	ctx, span := otel.Tracer("go-apk").Start(ctx, "installRegAndLinks")
	defer span.End()
...
func (a *APK) installSymlinksAndDirs(ctx context.Context, pkg *repository.RepositoryPackage, expanded *APKExpanded) (map[string]tar.Header, error) { //nolint:gocyclo
	_, span := otel.Tracer("go-apk").Start(ctx, "installSymlinksAndDirs")
	defer span.End()

	log.Printf("symlinks and dirs for %s", pkg.Name)
	...
	for {
		...
		case tar.TypeDir:
			...
			installed = append(installed, header)
		case tar.TypeSymlink:
			// some underlying filesystems and some memfs that we use in tests do not support symlinks.
			// attempt it, and if it fails, just copy it.
			// if it already exists, pointing to the same target, we can ignore it
	...
	expanded.InstalledFiles = append(expanded.InstalledFiles, installed...)
